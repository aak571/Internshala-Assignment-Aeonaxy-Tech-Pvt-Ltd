...
const BuildProfile = () => {
    const [profile_details, set_profile_details] = useState({ location: '', what_brings_you_here: '' })
    ...
    const finish_onclick_handler = async () => {
        const { location, what_brings_you_here } = profile_details
        // const image = profile_image.image
        const { name, type } = profile_image.image
        await axios.post('http://localhost:5000/api/v1/user/profile_build', { location, what_brings_you_here, username: 'aakukavi', name, type })
            .then(res => {
                cl(res)
            })
            .catch(err => {
    ...
    return (
        <div className="d-flex flex-column col-sm-6 mx-auto mt-5">
            ...
            <input id="what_brings_you_here" onChange={profile_details_onchange_handler} placeholder="what brings you here ?" className="mt-5"></input>
